...
export const register = async ({email, password, firstName, lastName, store, phoneNumber, dispatch }: RegisterProps) => {
    ...
    await request({
        ...
        getAccount({email, accessToken: response.data.accessToken, refreshToken: response.data.refreshToken, store, dispatch});
    }).catch((error) => {
